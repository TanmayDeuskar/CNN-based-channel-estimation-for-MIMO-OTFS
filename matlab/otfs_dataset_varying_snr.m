...
for t = 1:Nt
    ...
    for p = 1:numPilots
        ...
        rx_frame = y_conv(startIdx:endIdx);   % length Lframe
...
for chunk = 1:numChunks
    ...
    for s = 1:thisChunk
        ...
        H = H_add_truth;   % or whichever H you want
        ...
        H_slice = H_add_truth(:, :, 1);   % take antenna = 1 slice
    ...
    outFile = fullfile(saveDir, sprintf('chunk_snr%02d.mat', SNRdB));
    save(outFile, 'HADD_chunk', 'yDD_chunk', 'feature_chunk', 'SNRdB', '-v7.3');
    fprintf('Saved %s\n', outFile);
end
...
function H_add = extract_H_ADD_from_channel_withPadding(chObj, M, N, Nt, Nfft, Fs, padding)
    ...
    for t = 1:Nt
        ...
        tx_dd(pilot_delay, :) = 1;         % excite all Doppler bins
        ...
        rx = squeeze(rx_multi(:,1));   % ensure vector
